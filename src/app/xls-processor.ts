...
export class XlsProcessor {
  ...
  async #generateCsv(sheet: WorkSheet & DenseWorkSheet, range: Range, headerRowData: HeaderRowData): Promise<ProcessResult> {
    const errors: string[] = [];
  ...
  *#getTransactions(sheet: WorkSheet & DenseWorkSheet, range: Range, headerRowData: HeaderRowData, errors: string[]): Iterable<RawTransaction> {
